...
$(function () {
  ...
  $("#group_id").on('change', function (e) {
    ...
    $("#ticket-title").on('change', function (e) {
        ...
        if(ticket_id != null) {
            $.ajax({
                url: "/ticket/get-assignee-by-ticket/"+ticket_id,
                type: 'GET',
                dataType: "json",
  ...
  if (addTaskBtn.length) {
    addTaskBtn.on('click', function (e) {
      ...
      sidebarLeft.removeClass('show');
      overlay.removeClass('show');
      return_combobox_multi('#ticket-title',"/ticket/get-ticket-incomplete", '');
      return_combobox_multi_custom("#task-assigned", "/ticket/get-assignee-group", "")
  ...
  if (newTaskForm.length) {
    ...
    newTaskForm.on('submit', function (e) {
      ...
      if (isValid) {
        ...
        $.ajax({
            url: "/ticket/update-assignee-ticket",
            type: 'POST',
            data: myform,
            dataType: "json",
            contentType: false,
...
function get_list_ticket(level,status,time) {
    ...
    $.ajax({
        ...
        processing: true,
        serverSide: true,
        url: `/ticket/getAssignTicket?level=${level}&status=${status}&time=${time}`,
        success: function (data) {
            if(data.length == 0) {
...
<input type="checkbox" ${element.status == 4 ||  element.status == 3 ? "disabled" : ""} class="form-check-input" data-id_ticket="${element.id}" ${ assignee_ids != null && assignee_ids.includes(idAuth) && element.status != 1   ? "checked" : ""}  id="customCheck16" />
