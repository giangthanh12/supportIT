...
$(function () {
  ...
  return_combobox_multi('#group_id_select',"/api/ticket/get-group", 'Danh sách nhóm hỗ trợ   ');
...
//   return_combobox_multi_custom("#task-assigned", "/ticket/get-assignee", "");
  ...
  if (addTaskBtn.length) {
    ...
    //   updateBtns.addClass('d-none');
      ...
      $('#name_creator').val(localStorage.getItem("auth_name")).attr("readonly", true);
      $('#email_creator').val(localStorage.getItem("auth_email")).attr("readonly", true);
      $("#cc").val("").change();
      $('#cc').select2({
          ajax: {
...
function get_list_ticket(level,status,time,group_id_filter) {
    ...
    $.ajax({
        ...
        // processing: true,
        // serverSide: true,
        url: `/api/ticket/getdata?level=${level}&status=${status}&time=${time}&group_id=${group_id_filter}`,
        beforeSend: function (xhr) {
            xhr.setRequestHeader('Authorization', 'Bearer '+localStorage.getItem("token"));
            xhr.setRequestHeader('Accept', 'application/json');
        },
