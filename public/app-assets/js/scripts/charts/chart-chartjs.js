...
//   })
  ...
  $.ajax({
    ...
    // ajax call data ticket not done
    $.ajax({
        type: "GET",
        url: "/get-ticketNotDone",
        dataType: "json",
        success: function (response) {
...
if ($(".date-group").length) {
    ...
    $.ajax({
        ...
        success: function (response) {
            let htmlResponse = "";
            $.each(response, function(index, item) {
            htmlResponse += `<tr class="text-truncate text-body collapsed" data-bs-toggle="collapse"
            data-bs-target="#accordionIcon-${item.id}" aria-controls="accordionIcon-${item.id}>
                                <td>
                                    <div style="text-align:left;"
                                        class="d-flex justify-content-left align-items-center">
                                        <div class="d-flex flex-column">
    ...
    if ($(".filter-group").length) {
        ...
        $.ajax({
            type: "GET",
            url: "/get-ticket-month?group_id="+group_id,
            dataType: "json",
            success: function (response) {
                barChartExample.data.datasets[0].data = response
                barChartExample.update();
...
function showListStaff(id_group) {
    $.ajax({
        type: "GET",
        url: "/get-statistic-staff/"+id_group,
        dataType: "json",
        success: function (response) {
            let htmlResponse = "";
            $.each(response, function(index, item) {
