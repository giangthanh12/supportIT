...
class CRest
{
	 ...
	 *
	 */
	protected static function callCurl($arParams, $infoToken)
	{
        ...
        if(is_null($infoToken)) {
            $arSettings = static::getAppSettings();
        } else {
        $arSettings = [
            "access_token"=>$infoToken["access_token"],
            "domain"=>$infoToken["domain"],
            "refresh_token"=>$infoToken["refresh_token"],
            "application_token"=>$infoToken["application_token"],
            "C_REST_CLIENT_ID"=>C_REST_CLIENT_ID,
            "C_REST_CLIENT_SECRET"=>C_REST_CLIENT_SECRET,
            "client_endpoint"=>"https://admin.sconnect.edu.vn/rest/"
        ];
        }
		...
		if($arSettings !== false)
		{
			...
			try
			{
				...
				if(!empty($result[ 'error' ]))
				{
					...
					{
						$result = static::GetNewAuth($arParams,$arSettings);
					}
					else
					{
						$arErrorInform = [
	...
	public static function call($method, $params = [], $storeToken=null)
	{
		...
		// }
		$result = static::callCurl($arPost, $storeToken);
		return $result;
	}
	...
	private static function GetNewAuth($arParams, $arSettings = null)
	{
		$result = [];
        if($arSettings == null) {
            $arSettings = static::getAppSettings();
		...
		if($arSettings !== false)
		{
			...
			$newData = static::callCurl($arParamsAuth,null);
			...
			if(static::setAppSettings($newData))
			{
				...
				$result = static::callCurl($arParams,null);
			}
		}
		return $result;
	}
	...
	private static function getAppSettings()
	{
		...
		else
		{
			...
			// get settingsdata lấy theo auth
			$arData = static::getSettingData();
            $arData[ 'client_endpoint' ] = "https://admin.sconnect.edu.vn/rest/";
			$isCurrData = false;
			if(
	...
	protected static function getSettingData()
	{
		...
		if(file_exists(__DIR__ . '\settings.php'))
		{
			...
			$return = static::expandData(file_get_contents(__DIR__ . '/setting.json'));
		...
		// trả về mảng trong setting bao gồm:
		//  [
        //     "C_REST_CLIENT_ID"
	...
	{
		return  (boolean)file_put_contents(__DIR__ . '/setting.json', static::wrapData($arSettings));
	}
