...
class TicketController extends Controller
{
    ...
    public function getAssigneeByTicket($ticket_id) {
        ...
        return $ticket->assignees_id;
    }
    public function getAssigneeGroup() {
        $group = Group::where('leader_id', Auth::id())->select("members_id as members")->first();
        return $group->members;
    }
    public function get_my_ticket(Request $request) {
        $status = $request->has("status") ? $request->get("status") : "";
        return view("my-ticket",compact("status"));
    }
    ...
    public function get_assign_ticket(Request $request) {
        $status = $request->has("status") ? $request->get("status") : "";
        return view("assign-ticket", compact("status"));
    }
    ...
    public function get_ticket_incomplete() {
    ...
    public function detail($id) {
        ...
        $comments = Comment::where("ticket_id", $id)->with("user:id,name,avatar")->get();
        if(!$ticket)
        return back();
        $leader_id = Group::find($ticket->group_id)->leader_id;
        return view("detail-ticket",compact("ticket","comments", "leader_id"));
    }
   ...
   public function confirm_ticket(Request $request) {
    ...
    // public function update_status(Request $request) { // đóng tác vụ or mở tác vụ
    //     $ticket = Ticket::find($request->id_ticket);
    //     $ticket->status = $ticket->status != 4 ? 4 : 2;
    //     $ticket->save();

    //     // notify thống bảo mở yêu cầu / đóng yêu cầu
    //     $desc = $ticket->status == 4 ? auth()->user()->name. " đã đóng yêu cầu" : auth()->user()->name. " đã mở lại yêu cầu";
    //     (new NotifyBitrix24())->sendLeaderAndRelatedMember($ticket, auth()->user()->storeToken, $desc);
    //     return response()->json([
    //         'msg'=>"Đã cập nhật trạng thái yêu cầu thành công",
    //         "data"=>$ticket
    //     ],Response::HTTP_OK);
    // }
